...
// Middleware to check if user has editor or instructor role
const requireEditor = (req, res, next) => {
  if (!req.user) {
    return res.status(401).json({ success: false, message: 'Authentication required' })
  }

  // Allow editor, instructor (Chief Editor), or admin roles
  if (req.user.role !== 'editor' && req.user.role !== 'instructor' && req.user.role !== 'admin') {
