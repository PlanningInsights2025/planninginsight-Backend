...
export const autoAssignSubmissions = async (req, res) => {
  try {
    ...
    // Get all editors
    const editors = await User.find({ role: 'editor' }).select('_id').lean()
    ...
    // Calculate assignments per editor
    const editorIds = editors.map(e => e._id)
    const assignmentsPerEditor = Math.ceil(allUnassigned.length / editorIds.length)
    
    let assignedCount = 0
    let editorIndex = 0
    
    // Distribute submissions evenly
    for (const submission of allUnassigned) {
      ...
      await submission.model.findByIdAndUpdate(submission.id, {
        ...
        assignedAt: new Date(),
        status: submission.type === 'paper' ? 'pending' : 'under-review'
      })
    ...
    res.json({
      success: true,
      message: `Successfully assigned ${assignedCount} submissions to ${editorIds.length} editors`,
      data: {
        assigned: assignedCount,
        editors: editorIds.length,
        avgPerEditor: Math.round(assignedCount / editorIds.length)
      }
    })
  } catch (error) {
